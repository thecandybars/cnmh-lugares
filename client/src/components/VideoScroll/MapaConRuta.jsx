...
const MapaConRuta = ({ pointA, pointB, progress, width, height, zoom }) => {
  const { isLoaded } = useLoadScript({
    googleMapsApiKey: "AIzaSyA_Hpw8rcmaKtIqU9wmEIpnHrZB2Tz-hv4",
  });
  ...
  return (
    <GoogleMap
      ...
      center={{
        lat: (pointA[1] + pointB[1]) / 2,
        lng: (pointA[0] + pointB[0]) / 2,
      }}
      zoom={zoom}
    >
      <Polyline
        path={[
          { lat: pointA[1], lng: pointA[0] },
          { lat: pointB[1], lng: pointB[0] },
        ]}
        options={{
          strokeColor: "#8e8a79",
          strokeOpacity: 1,
      ...
      />
      <Marker position={{ lat: pointA[1], lng: pointA[0] }} />
      <Marker position={{ lat: pointB[1], lng: pointB[0] }} />
